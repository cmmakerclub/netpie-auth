...
export class NetpieAuth {
  ...
  async initSync() {
    ...
    let should_revoke = ((this.appid !== appid_cached) ||
      (this.appkey !== appkey_cached) ||
      (this.appsecret !== appsecret_cached) && appid_cached && appkey_cached &&
      appsecret_cached);
  ...
  getMqttAuth = async (callback) => {
    if (this._storage.get(Storage.KEY_STATE) ===
      Storage.STATE.STATE_ACCESS_TOKEN) {
      ...
      let [access_token, access_token_secret] = [
        this._storage.get(Storage.KEY_ACCESS_TOKEN),
      ...
      let mqttpassword = Util.compute_mqtt_password(access_token,
        mqttusername,
        hkey);
      let [input, protocol, host, port] = endpoint.match(
        /^([a-z]+):\/\/([^:\/]+):(\d+)/);
    ...
    } else if (this._storage.get(Storage.KEY_STATE) ===
      Storage.STATE.STATE_REQ_TOKEN) {
  ...
  _getAccessToken = async () => {
    return this.buildRequestObject("/api/atoken")
      ...
      .request((request_data) => {
        ...
        let auth_header = this.oauth.toHeader(this.oauth.authorize(request_data,
          _reqtok)).Authorization;
        return auth_header;
      });
  };
  ...
  getOAuthToken = async () => {
    ...
    if (req1_resp.ok) {
      ...
      let hkey = Util.compute_hkey(access_token.oauth_token_secret,
        this.appsecret);
      let revoke_token = Util.compute_revoke_code(access_token.oauth_token,
        hkey);
