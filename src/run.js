...
let hmac = (text, key) => base64(CryptoJS.HmacSHA1(text, key))
let base64 = (text) => text.toString(CryptoJS.enc.Base64)

let compute_hkey = (access_token_secret, app_secret) => `${access_token_secret}&${app_secret}`
let compute_mqtt_password = (access_token, mqttusername, hkey) => hmac(`${access_token}%${mqttusername}`, hkey)
let compute_revoke_code = (access_token, hkey) => hmac(access_token, hkey).replace(/\//g, '_');
